...
SceneFileParser.prototype.getHamburgers = function (hamburgers) {
    if(this.mScene.Hamburgers !== null){
        for(var i = 0; i < this.mScene.Hamburgers.length; i++){
            var hamburgersAttr = [];

            var hamburgersPos = this.mScene.Hamburgers[i].Position;
            var hamburgersScale = this.mScene.Hamburgers[i].Scale;
            hamburgersAttr = [hamburgersPos[0], hamburgersPos[1], hamburgersScale[0], hamburgersScale[1]];
            hamburgers.push(hamburgersAttr);
        }
    }
    return hamburgers;
};
SceneFileParser.prototype.getBackgs = function (backgs) {
    for(var i = 0; i < this.mScene.Backgs.length; i++){
        var backgAttr = [];

        var backgPos = this.mScene.Backgs[i].Position;
        var backgScale = this.mScene.Backgs[i].Scale;
        backgAttr = [backgPos[0], backgPos[1], backgScale[0], backgScale[1]];
        backgs.push(backgAttr);
    }
    return backgs;
};
